...
export default function GamePage({ initialLobby }: { initialLobby: Game }) {
  ...
  useEffect(() => {
    if (
      ...
      !lobby.white ||
      !lobby.black ||
      (lobby.white.id !== session?.profile?.id &&
        lobby.black.id !== session?.profile?.id)
    )
      return
  ...
  useEffect(() => {
    if (!session?.profile || !session.profile?.id) return
    socket.connect()
    ...
    syncSide(session.profile, undefined, lobby, { updateLobby })

    initSocket(session.profile, socket, lobby, {
      updateLobby,
      addMessage,
      updateCustomSquares,
      makeMove,
  ...
  function sendChat(message: string) {
    ...
    addMessage({ author: session.profile, message })
  }
  ...
  function getPlayerHtml(side: "top" | "bottom") {
      ...
      <div className="flex w-full flex-col justify-center">
        ...
        <span className="flex items-center gap-1 text-xs">
          ...
          {lobby.black?.connected === false && (
            <span className="badge-error badge badge-xs">disconnected</span>
          )}
        </span>
      </div>
    )
    ...
    if (lobby.black?.id === session?.profile?.id) {
  ...
  function copyInvite() {
    const text = `https://${process.env.NEXT_APP_URL || "not defined yet"}/${
    ...
    return movePairs.map((moves, i) => {
      return (
        <tr className="flex w-full items-center gap-1" key={i + 1}>
          ...
          <td
            className={
              "btn-ghost btn-xs btn h-full w-2/5 font-normal normal-case" +
  ...
  return (
    <div className="flex w-full flex-wrap justify-center gap-6 p-4 lg:gap-10 2xl:gap-16">
          ...
          <div className="flex flex-1 flex-col gap-1">
            <div className="mb-2 flex w-full flex-col items-end gap-1">
              ...
              >
                ...
                >
                  <IconCopy size={16} />$
                  {process.env.NEXT_APP_URL || "not defined yet"}/
                  {lobby.endReason ? `archive/${lobby.id}` : initialLobby.code}
                </label>
                <div
                  tabIndex={0}
            ...
            <div className="flex h-4 w-full">
              <button
                className={
                  "btn-sm btn grow rounded-r-none" +
              ...
              <button
                className={
                  "btn-sm btn grow rounded-l-none" +
                  (navIndex === null ? " btn-disabled" : "")
                }
              ...
              {lobby.endReason ? (
                <div>
                  ...
                  <a
                    ...
                    rel="noopener noreferrer"
                  >
                    ${process.env.NEXT_APP_URL || "not defined yet"}/archive/
                    {lobby.id}
                  </a>
                  .
                </div>
          ...
          <div className="bg-base-300 flex h-full w-full min-w-[64px] flex-col rounded-lg p-4 shadow-sm">
            ...
            <form className="input-group mt-auto" onSubmit={chatClickSend}>
              <input
                ...
                required
              />
              <button className="btn-secondary btn ml-1" type="submit">
                send
              </button>
            </form>
          </div>
