...
export const save = async (game) => {
    try {
        ...
        if (black.id || white.id) {
            ...
            if (game.winner === "draw") {
                if (white.id) {
                    await db.query(`UPDATE "user" SET draws = draws + 1 WHERE id = $1`, [
                        white.id,
                    ]);
                }
                if (black.id) {
                ...
                if (winner.id) {
                    await db.query(`UPDATE "user" SET wins = wins + 1 WHERE id = $1`, [
                        winner.id,
                    ]);
                }
                if (loser.id) {
                    await db.query(`UPDATE "user" SET losses = losses + 1 WHERE id = $1`, [loser.id]);
                }
            }
        }
        return {
    ...
    try {
        ...
        if (res.rowCount) {
            return {
                ...
                white: {
                    id: res.rows[0].white_id || undefined,
                    name: res.rows[0].white_name,
                },
                black: {
                    id: res.rows[0].black_id || undefined,
                    name: res.rows[0].black_name,
                },
                startedAt: res.rows[0].started_at.getTime(),
                endedAt: res.rows[0].ended_at?.getTime() || undefined,
            };
...
export const remove = async (id) => {
    try {
        const res = await db.query(`DELETE FROM "game" WHERE id = $1 RETURNING *`, [
            id,
        ]);
        return {
            id: res.rows[0].id,
